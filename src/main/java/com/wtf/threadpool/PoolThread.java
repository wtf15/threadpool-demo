...
public class PoolThread extends Thread {
	...
	public void run() {
		while(idleLocal!=null && idleLocal.get()){
			logger.info("{} my task is |{}", this.getName(), task);
			if(task!=null){
				try{
					idleLocal.set(false);
					...
					synchronized (this) {
						...
						if(pool.isShutdown()){
							logger.info("find thread pool shutdown, I am going die......");
							break;
						}
	...
	public void setTask(Task task) {
		logger.info("{} set the task|{} now ...",this.getName(), task);
		this.task = task;
		synchronized (this) {
			logger.info("{} set the task synchronized ...", this.getName());
			this.notifyAll();
		}
	}
